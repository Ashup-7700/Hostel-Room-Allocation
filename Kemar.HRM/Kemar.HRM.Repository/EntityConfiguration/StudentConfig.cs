...
namespace Kemar.HRM.Repository.EntityConfiguration
{
    internal class StudentConfig : BaseEntityConfig<Student>, IEntityTypeConfiguration<Student>
    {
        public override void Configure(EntityTypeBuilder<Student> builder)
        {
            ...
            builder.Property(s => s.Phone).IsRequired(false).HasMaxLength(15);
            builder.Property(s => s.Email).IsRequired().HasMaxLength(120);
            builder.Property(s => s.Address).IsRequired(false).HasMaxLength(200);
            builder.Property(s => s.DateOfAdmission).IsRequired();

            builder.HasMany(s => s.RoomAllocations).WithOne(ra => ra.Student).HasForeignKey(ra => ra.StudentId).OnDelete(DeleteBehavior.Restrict);
            builder.HasMany(s => s.Payments).WithOne(p => p.Student).HasForeignKey(p => p.StudentId).OnDelete(DeleteBehavior.Restrict);
